...
// CAMERA_ZOOM/VIEWPORT_COLS/VIEWPORT_ROWS는 테스트 빌드가 런타임에 덮어쓸 수 있어서
// (core/viewportConfig.js) 여기서 직접 import하지 않고 매번 getViewportConfig()로 읽는다.
...
export function toScreenX(gridX, camera) {
  return (gridX - camera.x) * CELL_SIZE * getViewportConfig().cameraZoom;
}

export function toScreenY(gridY, camera) {
  return (gridY - camera.y) * CELL_SIZE * getViewportConfig().cameraZoom;
}

// 화면에 그려지는 칸 하나의 실제 픽셀 크기(확대 배율 반영) - CELL_SIZE를 직접 쓰던 자리를
// 전부 이걸로 바꾼다.
export function screenCellSize() {
  return CELL_SIZE * getViewportConfig().cameraZoom;
}
...
// 테스트 빌드가 CAMERA_ZOOM/VIEWPORT_COLS/ROWS를 덮어써도 이 값 자체는 항상 main.js가
// 페이지 로딩 시 고정한 실제 캔버스 크기(현재 768x576)와 같아야 한다 - testBuilds.js의
// viewportOverrides를 작성할 때 그렇게 되도록 값을 맞춰야 한다(viewportConfig.js 참고).
export function getViewportPixelSize() {
  const { cameraZoom, viewportCols, viewportRows } = getViewportConfig();
  return {
    width: viewportCols * CELL_SIZE * cameraZoom,
    height: viewportRows * CELL_SIZE * cameraZoom,
  };
}
