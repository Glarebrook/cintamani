...
**`World` 객체**: `game.js`는 하나의 `world` 객체(`{ snake, enemyManager, itemManager, projectileManager, eventBus, stats, startTime, reset() }`)를 만들어 상태/시스템/매니저에 참조로 넘깁니다. 게임을 재시작해도 `world` 객체 자체를 새로 만들지 **않습니다** — `world.reset()`을 호출해서 `world`의 필드들을 제자리에서 다시 채웁니다(새 `Snake`, 새 `EnemyManager` 등). 이게 중요한 이유: 상태(state)와 시스템은 반드시 호출 시점에 `world.snake` / `world.enemyManager` 등을 통해 읽어야지, 생성 시점에 그 참조를 구조분해해서 캐싱해두면 재시작이 반영되지 않습니다. `world.stats`(`{ tickMs, attackDamage, enemyKillStacks }`)는 예전엔 `TICK_MS`/`PROJECTILE_DAMAGE` 상수를 직접 읽던 값들의 "이번 판 현재값"을 들고 있습니다 — 그 상수들은 이제 재시작할 때 되돌아가는 기본값일 뿐이고, 실시간 값이 필요한 곳(게임 루프의 틱 간격, `fire()`의 발사체 공격력, HUD)은 전부 `world.stats.*`를 읽어야 합니다. `speedUp`/`speedDown`/`attackUp` 같은 아이템이 게임 도중 이 값을 바꾸기 때문입니다. `core/loop.js`가 고정된 `tickMs` 대신 `getTickMs()` 콜백을 받는 것도 같은 이유입니다 — 루프가 이미 시작된 뒤에도 이동 간격이 바뀔 수 있습니다. `enemyKillStacks`는 `{ [적타입id]: 처치수 }` 형태의 단순한 맵으로, `world.reset()`마다 새로 초기화되며, 어떤 적 타입의 `onDefeated` 훅에서든 여기에 누적시킬 수 있습니다(`chaser.js` 참고) — 특정 타입에 종속되지 않고 범용으로 만들어서, 나중에 4번/5번적도 비슷한 처치-보상 방식을 쓸 때 각자 필드를 새로 만들 필요가 없습니다.
...
**게임 상태 머신** (`core/stateMachine.js` + `states/*.js`): 현재는 `title`, `playing`, `gameOver` 세 가지이며, 시작 상태는 `playing`이 아니라 `title`입니다. 각 상태는 `{ enter(payload), exit(), onFrame(dt), onTick(), render() }` 형태입니다. `game.js`의 루프는 항상 `stateMachine.current`에 위임합니다. 각 상태는 공용 `input/actions.js` 싱글턴을 통해 `enter()` 시점에 자기 키 액션을 바인딩하고 `exit()` 시점에 해제합니다(예: `playingState`는 Space→발사를 바인딩하고, `gameOverState`는 Enter→(바로 재시작이 아니라) 타이틀 화면 복귀를 바인딩). `titleState`는 `Actions.bindAny(onStart)`/`unbindAny()`를 씁니다 — 특정 키에 매칭되는 핸들러가 없을 때만 대신 걸리는 "아무 키" 폴백 바인딩으로, "아무 키나 누르면"은 `bind(key, ...)` 하나로는 표현할 수 없어서 추가했습니다. 실제로 `world.reset()`을 호출하고 `playing`으로 전환하는 건 `onStart`입니다. 그래서 어디에도 `if (gameOver)` 같은 명시적 체크 없이도 Enter 재시작이 `gameOver` 상태일 때만 동작하는 것이고, 흐름은 항상 타이틀 → 플레이 → 게임오버 → 타이틀이며 게임오버에서 플레이로 바로 가는 경로는 없습니다.
...
- **적 타입** (`content/enemies/`): config 필드는 `color, hp, canBeDamagedByProjectile, displayText(enemy), collidesWithHead`이며, 선택적으로 `spawnEligible(world)`(그 타입이 지금 스폰 가능한지 게이트 — 예: `sentinel.js`의 타입은 뱀이 충분히 길어져야만 가능해짐), `captureZone: { scale }`(아래의 포위 메커니즘에 그 타입을 opt-in시킴), `move(enemy, { snake })` + `moveIntervalMs(enemy, { snake })`(그 타입을 자가 이동에 opt-in시킴 — `chaser.js`가 첫 번째로 스스로 움직이는 적입니다: 각각 `{x,y}` 방향 한 칸과 현재 이동 간격(ms)을 반환합니다. `managers/enemyManager.js`의 `updateMovement`가 이 두 필드를 가진 타입이면 무엇이든 범용으로 호출하며, 적 인스턴스별 `moveTimer`를 누적시킵니다 — `basic`/`sentinel`처럼 정적인 타입은 그냥 `move`를 정의하지 않아서 건너뛰어집니다). `updateMovement`는 `onFrame(dt)`에서 실제 프레임 dt로 호출되며, `onTick`에서 뱀의 틱 크기 dt로 호출되는 스폰 타이머용 `update(dt, snake)`와는 의도적으로 분리돼 있습니다 — 이동하는 적의 간격(예: chaser가 추격 중일 때 60ms)이 뱀 자신의 틱 간격보다 짧을 수 있는데, 틱마다 한 번만 이동을 갱신하면 어떤 적이든 "틱당 최대 한 칸"이라는 상한에 묶여버려서 속도 차이가 아예 드러나지 않게 됩니다), `onDefeated(world, enemy)`(이 적 인스턴스가 투사체로 처치됐을 때만, 딱 한 번 호출됩니다 — `managers/projectileManager.js` 참고. 포획으로 제거될 때는 호출되지 않습니다, 완전히 다른 제거 경로라서요. `chaser.js`는 이걸로 `world.stats.enemyKillStacks`에 타입별 처치 수를 누적하다가 `CHASER_KILL_STACK_THRESHOLD`마다 보너스 아이템을 스폰합니다), `onCaptured(world, enemy)`(`onDefeated`의 포획 버전 — 포위 메커니즘으로 제거된 적마다 한 번씩 호출됩니다. `states/playingState.js`의 `onTick()`이 `enemyManager.removeByIds()` 바로 다음에 호출하는데, 이 메서드가 바로 그 용도로 제거된 적 객체들을 반환합니다. `turret.js`가 확률 기반 아이템 드랍에 씁니다), `ability(enemy, world)` + `abilityCooldownMs(enemy, world)`(`move`/`moveIntervalMs`와 같은 모양이지만 위치 이동이 아니라 그 외의 시간차 부수효과를 opt-in시킴 — `turret.js`가 뱀이 같은 행/열에 정렬됐을 때 투사체를 쏘는 데 씁니다. `enemyManager.js`의 `updateAbilities(dt, world)`가 범용으로 호출하며, 이동과 같은 이유로 역시 `onFrame`에서 호출됩니다. 쿨다운은 "발사" 자체에만 걸립니다, 조건 체크에는 안 걸립니다 — 타이머가 쿨다운을 넘기고 나면 `updateAbilities`는 `ability()`가 참값을 반환할 때까지 매 프레임 계속 호출하고, 참값을 반환했을 때만 타이머를 리셋합니다. 만약 쿨다운이 다 찬 바로 그 순간에만 조건을 딱 한 번 샘플링하는 식이었다면, 하필 그 순간에 정렬이 안 돼 있으면 몇 초씩 아예 안 쏘는 것처럼 보이게 됩니다. 그래서 `ability()`는 실제로 발동했는지를 반환값으로 알려줘야 합니다)를 가질 수 있습니다. `managers/enemyManager.js`는 절대 특정 type id를 특별 취급하지 않습니다. 적 인스턴스는 원시 type id가 아니라, 스폰 시점에 한 번 resolve된 `typeDef` 참조를 들고 있습니다. `checkHeadCollision`은 적의 정중앙 1칸이 아니라 `render()`가 그리는 `ENEMY_SCALE × ENEMY_SCALE` 전체 박스를 히트박스로 취급합니다 — 그려지는 크기보다 히트박스가 작으면 플레이어 입장에서는 "가끔 안 먹힌다"로 보이기 때문에 일부러 render()의 박스와 맞춰뒀습니다.
- **아이템** (`content/items/`): config 필드는 `color`와 `onPickup(world, item)`이고, 선택적으로 `spawnEligible(world)`(적 타입과 동일한 게이팅 패턴 — `tripleGrowth.js`와 `attackUp.js` 둘 다 이걸 항상 `() => false`로 고정해서 무작위 스폰 풀에서 영구히 제외시킵니다. 각각 `chaser.js`의 처치 스택 보상, `turret.js`의 포획 드랍 확률로만 등장하기 때문입니다)와 `overlayText`(아이템 칸 위에 작은 흰 글씨로 겹쳐 그리는 텍스트, 예: `tripleGrowth.js`의 `'x3'` — 적의 `displayText`와 같은 역할)를 가질 수 있습니다. `managers/itemManager.js`는 `foods` 배열(최대 `FOOD_MAX_COUNT`개 동시 존재)을 관리하며, 그 개수보다 적을 때 `FOOD_INTERVAL_MS` 타이머마다 하나씩 스폰합니다. `spawnItem`은 공용 `_findFreeCell` 헬퍼를 통해 `spawnEligible`을 통과한 등록된 타입들 중에서 균등 확률로 무작위로 고릅니다(특정 아이템 id를 하드코딩하지 않음). `spawnSpecific(typeId, snake, enemyManager)`은 자격 필터와 무작위 선택을 둘 다 건너뛰고 지정한 타입 하나를 즉시 스폰합니다 — 일반 타이머 밖에서(예: 적의 `onDefeated`/`onCaptured`) 트리거되는 보상형 스폰이 이 경로를 씁니다. `checkHeadCollision`은 격자 좌표로 매칭해서 먹은 것 하나만 배열에서 빼냅니다. 스탯을 바꾸는 아이템(`speedUp`/`speedDown`/`attackUp`)은 상수가 아니라 `world.stats`를 직접 바꾸므로, 그 효과가 판이 끝날 때까지 유지됩니다.
